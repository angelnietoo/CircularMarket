...
public interface ProductoRepository extends JpaRepository<Producto, Long> {
    ...
    @Query("""
           ...
           WHERE
                ...
                OR LOWER(CAST(p.id AS string)) LIKE LOWER(CONCAT('%', :busqueda, '%'))

                OR (
                    LOWER(:busqueda) IN ('activo', 'activa')
                    AND p.activo = true
                )

                OR (
                    LOWER(:busqueda) IN ('inactivo', 'inactiva')
                    AND p.activo = false
                )
           ...
           """)
    Page<Producto> buscarProductosAdmin(@Param("busqueda") String busqueda, Pageable pageable);
}